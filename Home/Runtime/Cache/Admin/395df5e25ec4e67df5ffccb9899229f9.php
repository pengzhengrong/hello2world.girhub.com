		...
		<div  class="wrap">
			<?php if(is_array($rest)): foreach($rest as $key=>$v): ?><div class="modules">
					<p>
						...
						<a href="<?php echo U('Admin/Rbac/role_add',array('pid'=>$v['id']) ,'');?>">ADD</a>|
						<a href="<?php echo U('Admin/Rbac/role_delete','id='.$v['id'] ,'' );?>">DELETE</a>
					</p>
				<?php if(is_array($v["child"])): foreach($v["child"] as $key=>$action): ?><div class="action">
						<p><?php echo ($action["name"]); ?>(<?php echo ($action["remark"]); ?>)
						<a href="<?php echo U('Admin/Rbac/role_add',array('pid'=>$action['id']) ,'');?>">ADD</a>|
						<a href="<?php echo U('Admin/Rbac/role_delete','id='.$action['id'] ,'' );?>">DELETE</a></p>
					<?php if(is_array($action["child"])): foreach($action["child"] as $key=>$method): ?><div class="method">
							...
							<a href="<?php echo U('Admin/Rbac/role_delete','id='.$method['id'] ,'' );?>">DELETE</a>
