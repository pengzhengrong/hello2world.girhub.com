...
function D($name='',$layer='') {
    ...
    // my_log( 'layer',$layer );  // layer = 'Model'
    if(strpos($name,'://')) {// 指定项目
        list($app)  =   explode('://',$name);
        $name       =   str_replace('://','/'.$layer.'/',$name);
    }else{
        $app        =   C('DEFAULT_APP');   //$app = '@'
        $name       =   $app.'/'.$layer.'/'.$name;  //$name= '@/Model/UserRelation'
    }
    ...
    if($list = C('EXTEND_GROUP_LIST') && isset($list[$app])){ // 扩展分组
        ...
        // my_log( 'import' , $name.$layer );
        import($name.$layer);   //import( @/Model/UserRelationModel )
    } 
    $class          =   basename($name.$layer);
    // my_log( 'class' , $class );   //$class = UserRelationModel
    // my_log( 'base_name' , basename( $name ) );   //base_name = UserRelation
...
function A($name,$layer='',$common=false) {
    ...
    $layer      =   $layer?$layer:C('DEFAULT_C_LAYER'); //$layer = Action $name=Rbac
    if(strpos($name,'://')) {// 指定项目
        list($app)  =   explode('://',$name);
        $name   =  str_replace('://','/'.$layer.'/',$name);
    }else{
        $app    =   '@';
        $name   =  '@/'.$layer.'/'.$name;  //$name=@/Action/Rbac
    }
    if(isset($_action[$name]))  return $_action[$name];
    $path           =   explode('/',$name); //$path = @  Action   Rbac
    // @error_log("\n EXTEND_GROUP_LIST=".EXTEND_GROUP_LIST ,3,"/tmp/pzrlog.log");
    if($list = C('EXTEND_GROUP_LIST') && isset($list[$app])){ // 扩展分组
        $baseUrl    =   $list[$app];
        import($path[2].'/'.$path[1].'/'.$path[3].$layer,$baseUrl);
    }elseif(count($path)>3 && 1 == C('APP_GROUP_MODE')) { // 独立分组
        ...
        // @error_log("\n name.layer=$name$layer",3,'/tmp/pzrlog.log');
        import($name.$layer);  //import @/Action/RbacAction
    }
    $class      =   basename($name.$layer);
    // @error_log("\n class=$class",3,'/tmp/pzrlog.log'); // $class=RbacAction
    if(class_exists($class,false)) {
        $action             =   new $class();  //$action = new RbacAction();
        $_action[$name]     =   $action;
        return $action;
    }else {
        return false;
