...
class Think {
    ...
    static private function buildApp() {
        ...
        if(defined('MODE_NAME')) { // 读取模式的设置
            $mode   = include MODE_PATH.strtolower(MODE_NAME).'.php';
             // @error_log("\n MODE_NAME=".$mode,3,'/tmp/pzrlog.log'); // no defined
        }else{
            $mode   =  array();
        }
        // 加载核心惯例配置文件
        ...
        // 加载项目配置文件
        //@error_log("\n CONF_PATH=".CONF_PATH,3,'/tmp/pzrlog.log'); // /Home/Conf
        ...
        // 加载项目公共文件
        // @error_log("\n COMMON_PATH=".COMMON_PATH,3,'/tmp/pzrlog.log'); // /Home/Common
    ...
    static public function appError($errno, $errstr, $errfile, $errline) {
      switch ($errno) {
          case E_ERROR: //can't handle by this function
          case E_PARSE:
          case E_CORE_ERROR:
