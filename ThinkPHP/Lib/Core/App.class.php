...
class App {
    ...
    static public function exec() {
        if(!preg_match('/^[A-Za-z](\w)*$/',MODULE_NAME)){ // 安全检测
            ...
            $module  =  A($group.MODULE_NAME);  //$module = new RbacAction();
             // @error_log("\n group=$group  MODULE_NAME=".MODULE_NAME,3,"/tmp/pzrlog.log"); 
             //MODULE_NAME='Rbac'  group=''
            // @error_log("\n module=$module",3,"/tmp/pzrlog.log"); //module=''
        }
        if(!$module) {
            // @error_log("\n module=$module",3,"/tmp/pzrlog.log"); //module='' not this way
            ...
            if(function_exists('__hack_module')) {
                ...
                // @error_log("\n module=".$module,3,"/tmp/pzrlog.log");  // not this way
                if(!is_object($module)) {
                    // 不再继续执行 直接返回
                    return ;
                }
        ...
        // @error_log("\n action=$action",3,"/tmp/pzrlog.log"); //action='access'
        try{
            ...
            // @error_log("\n method=$method",3,"/tmp/pzrlog.log");
            if($method->isPublic()) {
                $class  =   new ReflectionClass($module);
                // @error_log("\n class=$class",3,"/tmp/pzrlog.log"); // Rbac's all method and reflect
                // 前置操作
                ...
                // URL参数绑定检测
                // @error_log("\n params_num=".$method->getNumberOfParameters(),3,"/tmp/pzrlog.log");  // params_num=0
                if(C('URL_PARAMS_BIND') && $method->getNumberOfParameters()>0){
                    ...
                    // @error_log("\n params={json_encode( $params)}",3,"/tmp/pzrlog.log"); 
